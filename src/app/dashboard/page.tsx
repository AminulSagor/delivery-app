...
export default function DashboardPage() {
  ...
  const successRatio = total ? ((totalDelivered / total) * 100).toFixed(1) : '0';
  ...
  return (
    <main className="min-h-screen bg-gray-50 px-4 py-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        <StatCard title="Success Ratio" value={`${successRatio}%`} bg="bg-green-100" text="text-green-700" />
        <StatCard title="Failed Ratio" value={`${failedRatio}%`} bg="bg-red-100" text="text-red-700" />
        <StatCard title="Return Ratio" value={`${returnRatio}%`} bg="bg-yellow-100" text="text-yellow-700" />
        <StatCard title="Total Riders" value={totalRiders} bg="bg-blue-100" text="text-blue-700" />
      </div>

      {/* Section Blocks */}
      <DashboardBlock title="➕ Add Daily Entry" description="Form will go here." />
      <DashboardBlock title="📆 Filter Entries" description="Date/rider filters will go here." />
      <DashboardBlock title="📊 Today's Entries">
        <TodayEntries entries={entries} />
      </DashboardBlock>
      <DashboardBlock title="📅 Monthly Report" description="Dropdown and Generate button." />
    </main>
  );
}
